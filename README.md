...
*Небольшое примечание: так как я не получил ответы на свои вопросы и не мог задать остальные, в спорных по моему мнению местах я действовал исходя из своих мыслей и логики. **Задание не является завершенным, так как 4-х дней оказалось недостаточно для такого объема работы.*** 
...
Так как нельзя хранить секреты в репозиториях, был создан `.env.example`, где описана структура итогового `.env` файла.
